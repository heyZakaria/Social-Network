...
func handleInvite(w http.ResponseWriter, r *http.Request) {
	...
	InsertQuery := `INSERT INTO group_invites (sender_id, reciever_id, group_id, Joinstate) VALUES (?, ?, ?, ?)`
...
func InviteValidation(senderId, receiverId, groupId string) (error, bool) {
	...
	err = db.DB.QueryRow(`SELECT EXISTS(SELECT 1 FROM follow_relationships WHERE follower_id = ? AND followed_id = ? AND status = 'accepted')`, senderId, receiverId).Scan(&isFriend)
	if err != nil {
		return err, false
	}
